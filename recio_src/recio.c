...
/*
 * If the file was a scratch file, delete it.
 */
...
{
  long recdif=recnum;   /* Record offset to desired position */
  int first = 1;        /* True only on first fseek() */
/*
 * Check args.
 */
  if(rec_bad(rio, "rec_write"))
 ...
 */
  if(recnum==rio->recnum && recoff==rio->recoff)
    return 0;
/*
 * Since fseek() requires that the file position be specified by a
 * (long) and a long can only contain numbers up to LONG_MAX, perform
 * the seek in steps of at most rio->reclim.
 */
  do {
    long newoff = recdif > rio->reclim ? rio->reclim : recdif;
    if(fseek(rio->fp, newoff * rio->reclen + (first ? recoff:0L), first ? SEEK_SET : SEEK_CUR)) {
      fprintf(stderr, "rec_seek: Error positioning file: %s\n", rio->name);
      return 1;
    };
/*
